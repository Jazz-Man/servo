...
fn test_domain_match() {
    assert!(StoredCookie::domain_match("foo.com", "foo.com"));
    assert!(StoredCookie::domain_match("bar.foo.com", "foo.com"));
    assert!(StoredCookie::domain_match("baz.bar.foo.com", "foo.com"));

    assert!(!StoredCookie::domain_match("bar.foo.com", "bar.com"));
    assert!(!StoredCookie::domain_match("bar.com", "baz.bar.com"));
    assert!(!StoredCookie::domain_match("foo.com", "bar.com"));

    assert!(!StoredCookie::domain_match("bar.com", "bbar.com"));
    assert!(StoredCookie::domain_match("235.132.2.3", "235.132.2.3"));
    assert!(!StoredCookie::domain_match("235.132.2.3", "1.1.1.1"));
    assert!(!StoredCookie::domain_match("235.132.2.3", ".2.3"));
}

#[test]
fn test_path_match() {
    assert!(StoredCookie::path_match("/", "/"));
    assert!(StoredCookie::path_match("/index.html", "/"));
    assert!(StoredCookie::path_match("/w/index.html", "/"));
    assert!(StoredCookie::path_match("/w/index.html", "/w/index.html"));
    assert!(StoredCookie::path_match("/w/index.html", "/w/"));
    assert!(StoredCookie::path_match("/w/index.html", "/w"));

    assert!(!StoredCookie::path_match("/", "/w/"));
    assert!(!StoredCookie::path_match("/a", "/w/"));
    assert!(!StoredCookie::path_match("/", "/w"));
    assert!(!StoredCookie::path_match("/w/index.html", "/w/index"));
    assert!(!StoredCookie::path_match("/windex.html", "/w/"));
    assert!(!StoredCookie::path_match("/windex.html", "/w"));
}

#[test]
fn test_default_path() {
    assert_eq!(&*StoredCookie::default_path("/foo/bar/baz/"), "/foo/bar/baz");
    assert_eq!(&*StoredCookie::default_path("/foo/bar/baz"), "/foo/bar");
    assert_eq!(&*StoredCookie::default_path("/foo/"), "/foo");
    assert_eq!(&*StoredCookie::default_path("/foo"), "/");
    assert_eq!(&*StoredCookie::default_path("/"), "/");
    assert_eq!(&*StoredCookie::default_path(""), "/");
    assert_eq!(&*StoredCookie::default_path("foo"), "/");
}
...
fn fn_cookie_constructor() {
    ...
    assert!(StoredCookie::new_wrapped(cookie.clone(), url, CookieSource::Http).is_some());
    let cookie = StoredCookie::new_wrapped(cookie, url, CookieSource::Http).unwrap();
    ...
    assert!(StoredCookie::new_wrapped(cookie.clone(), url, CookieSource::Http).is_none());
    assert!(StoredCookie::new_wrapped(cookie, gov_url, CookieSource::Http).is_some());

    // cookie domain matching test
    let cookie = cookie::Cookie::parse(" baz = bar ; Secure; Domain = bazample.com").unwrap();
    assert!(StoredCookie::new_wrapped(cookie, url, CookieSource::Http).is_none());

    let cookie = cookie::Cookie::parse(" baz = bar ; Secure; Path = /foo/bar/").unwrap();
    assert!(
        StoredCookie::new_wrapped(cookie, url, CookieSource::Http).is_none(),
    ...
    assert!(StoredCookie::new_wrapped(cookie, url, CookieSource::NonHttp).is_none());

    let secure_url = &Url::parse("https://example.com/foo").unwrap();
    let cookie = cookie::Cookie::parse(" baz = bar ; Secure; Path = /foo/bar/").unwrap();
    let cookie = StoredCookie::new_wrapped(cookie, secure_url, CookieSource::Http).unwrap();
    ...
    assert!(StoredCookie::new_wrapped(cookie, u, CookieSource::Http).is_some());

    let cookie = cookie::Cookie::parse("foo=bar; max-age=99999999999999999999999999999").unwrap();
    let cookie = StoredCookie::new_wrapped(cookie, u, CookieSource::Http).unwrap();
    assert!(
        cookie
            .expiry_time
...
fn test_cookie_secure_prefix() {
    ...
    let url = &Url::parse("https://example.com").unwrap();
    let cookie = cookie::Cookie::parse("__Secure-SID=12345; Secure").unwrap();
    assert!(StoredCookie::new_wrapped(cookie, url, CookieSource::Http).is_some());
...
fn test_sort_order() {
    ...
    let a = StoredCookie::new_wrapped(a_wrapped.clone(), url, CookieSource::Http).unwrap();
    delay_to_ensure_different_timestamp();
    let a_prime = StoredCookie::new_wrapped(a_wrapped, url, CookieSource::Http).unwrap();
    let b = cookie::Cookie::parse("baz=bar;Path=/foo/bar/baz/").unwrap();
    let b = StoredCookie::new_wrapped(b, url, CookieSource::Http).unwrap();
...
fn add_cookie_to_storage(storage: &mut CookieStorage, url: &Url, cookie_str: &str) {
    let source = CookieSource::Http;
    let cookie = cookie::Cookie::parse(cookie_str.to_owned()).unwrap();
    let cookie = StoredCookie::new_wrapped(cookie, url, source).unwrap();
...
fn assert_ip_cookie_bucket_collision_eviction(ip_a: &str, ip_b: &str) {
    ...
    let ip_a = Url::parse(ip_a).unwrap();
    let ip_b = Url::parse(ip_b).unwrap();
    let source = CookieSource::Http;
...
fn test_insecure_cookies_cannot_evict_secure_cookie() {
    let mut storage = CookieStorage::new(5);
    let secure_url = Url::parse("https://home.example.org:8888/cookie-parser?0001").unwrap();
    let source = CookieSource::Http;
    ...
    for bare_cookie in cookies {
        let cookie = StoredCookie::new_wrapped(bare_cookie, &secure_url, source).unwrap();
        storage.push(cookie, &secure_url, source);
    }

    let insecure_url = Url::parse("http://home.example.org:8888/cookie-parser?0001").unwrap();
    ...
    let url =
        Url::parse("https://home.example.org:8888/foo/cookie-parser-result?0001").unwrap();
    let source = CookieSource::Http;
    assert_eq!(
    ...
    let url =
        Url::parse("https://home.example.org:8888/foo/bar/cookie-parser-result?0001").unwrap();
    let source = CookieSource::Http;
    assert_eq!(
        storage.cookies_for_url(&url, source).unwrap(),
...
fn test_secure_cookies_eviction() {
    let mut storage = CookieStorage::new(5);
    let url = Url::parse("https://home.example.org:8888/cookie-parser?0001").unwrap();
    let source = CookieSource::Http;
    let mut cookies = Vec::new();
    ...
    for bare_cookie in cookies {
        let cookie = StoredCookie::new_wrapped(bare_cookie, &url, source).unwrap();
        storage.push(cookie, &url, source);
    }
...
fn add_retrieve_cookies(
    ...
    for str_cookie in set_cookies {
        let cookie = StoredCookie::from_cookie_string(str_cookie, &url, source).unwrap();
        storage.push(cookie, &url, source);
    }
...
fn test_parse_date() {
    assert_eq!(
        StoredCookie::parse_date("26 Jun 2024 15:35:10 GMT"), // without day of week
        Some(datetime!(2024-06-26 15:35:10).assume_utc())
    );
    assert_eq!(
        StoredCookie::parse_date("26-Jun-2024 15:35:10 GMT"), // dashed
        Some(datetime!(2024-06-26 15:35:10).assume_utc())
    );
    assert_eq!(
        StoredCookie::parse_date("26 Jun 2024 15:35:10"), // no GMT
        Some(datetime!(2024-06-26 15:35:10).assume_utc())
    );
    assert_eq!(
        StoredCookie::parse_date("26 Jun 24 15:35:10 GMT"), // 2-digit year
        Some(datetime!(2024-06-26 15:35:10).assume_utc())
    );
    assert_eq!(
        StoredCookie::parse_date("26 jun 2024 15:35:10 gmt"), // Lowercase
        Some(datetime!(2024-06-26 15:35:10).assume_utc())
    );
}
...
fn test_clear_storage_without_url_clears_everything() {
    ...
    let source = CookieSource::Http;
    let url = Url::parse("http://example.com/").unwrap();
    let other_url = Url::parse("http://example.org/").unwrap();
